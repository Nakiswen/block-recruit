...
export class SmartFilterBuilder {
  ...
  private static buildCandidateToJobFilter(
    ...
    if (
      ...
      candidateData.location !== '未知' &&
      candidateData.location !== 'Not specified'
    ) {
      orConditions.push({
        $or: [
    ...
    switch (matchMode) {
      ...
      case 'flexible':
        ...
        if (typeof minimumMatch === 'number' && minimumMatch <= skills.length) {
          ...
          skillConditions.push({
            $or: [{ [primaryField]: { $in: skills } }, { [secondaryField]: { $in: skills } }],
          });
        }
        break;
   ...
   * 构建相似职位搜索的过滤器
   */
  private static buildSimilarJobsFilter(
    jobData?: FilterBuilderParams['jobData']
  ): Record<string, any> | undefined {
    if (!jobData) return undefined;
   ...
   * 构建相似求职者搜索的过滤器
   */
  private static buildSimilarCandidatesFilter(
    candidateData?: FilterBuilderParams['candidateData']
  ): Record<string, any> | undefined {
    if (!candidateData) return undefined;
  ...
  static quickFilters = {
    ...
    // 远程工作过滤器
    remoteJobs: () => ({
      $or: [{ location: '远程' }, { location: 'Remote' }, { remote_allowed: true }],
    }),
          ...
          if ('min' in value && 'max' in value) {
            if (typeof value.min === 'number' && typeof value.max === 'number') {
              if (value.min > value.max) {
                errors.push(
                  `薪资范围无效: 最小值(${value.min})大于最大值(${value.max}) 在路径 ${currentPath}`
                );
              }
    ...
    const checkSkills = (obj: any, path = '') => {
      for (const [key, value] of Object.entries(obj)) {
        ...
        if (key.includes('skill') && Array.isArray(value)) {
          ...
          if (value.length > 50) {
            warnings.push(
              `技能列表过长(${value.length}项), 可能影响查询性能 在路径 ${currentPath}`
            );
          }
  ...
  static describeFilter(filter: Record<string, any>): string {
    ...
    const describe = (obj: any, context = ''): void => {
      ...
      for (const [key, value] of Object.entries(obj)) {
        if (key === 'experience_years') {
          ...
          if (isRecord(value) && value.min && value.max) {
          ...
          } else if (isRecord(value) && value.$in) {
    ...
    return descriptions.length > 0 ? `筛选条件: ${descriptions.join(' · ')}` : '无特定筛选条件';
  }
}
