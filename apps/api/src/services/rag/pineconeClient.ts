...
export class PineconeClient {
   ...
   */
  static async upsert(item: VectorItem, indexType?: PineconeIndexType): Promise<{ 
    success: boolean; 
    count: number;
    data?: any;
    error?: string;
    ...
    try {
      ...
      console.log(`${success ? '✅' : '❌'} ${actualIndexType} 向量 ${item.id} ${success ? '上传成功' : '上传失败'}`);
   ...
   * @returns 操作结果对象，包含成功状态和详细信息
   */
  static async batchUpsert(items: VectorItem[], batchSize = 100): Promise<{ 
    success: boolean;
    count: number;
    data?: any;
    error?: string;
    ...
    if (jobVectors.length > 0) {
      try {
        ...
        hasError = true;
        errors.push(`岗位向量批量插入发生异常: ${error instanceof Error ? error.message : '未知错误'}`);
      }
    }
    ...
    if (resumeVectors.length > 0) {
      try {
        const result = await this.batchUpsertToIndex(resumeVectors, PineconeIndexType.RESUME, batchSize);
        successCount += result.success ? result.count : 0;
        if (!result.success) {
          hasError = true;
        ...
        hasError = true;
        errors.push(`简历向量批量插入发生异常: ${error instanceof Error ? error.message : '未知错误'}`);
      }
    }
  ...
  static async search(params: PineconeSearchParams): Promise<{
    ...
    try {
      ...
      // 处理和标准化匹配结果
      const matches = response.matches?.map((match,i) => {
        console.log(`\n${i + 1}. ${match.id}`);
      ...
      console.log(`✅ ${indexType} 向量搜索完成，找到 ${matches.length} 条结果，用时 ${searchTimeMs}ms`);
    ...
    try {
      ...
      for (let i = 0; i < vectors.length; i += batchSize) {
        ...
        try {
          ...
          console.log(`✅ 批次 ${Math.floor(i/batchSize) + 1}/${Math.ceil(vectors.length/batchSize)} 插入成功: ${batch.length} 条`);
        } catch (error) {
