 ...
 * 传统的结构化数据提取服务（不使用AI）
 */
import { aiService, JobStructuredInfo, ResumeStructuredInfo } from '@/services/ai/aiService.js';
import { Job, Resume } from './types.js';
...
class TraditionalExtractionService {
  ...
  extractJobInfo(job: Job): JobStructuredInfo {
    return {
      ...
      parsedRequiredSkills: this.extractRequiredSkills(job),
      parsedPreferredSkills: this.extractPreferredSkills(job),
  ...
  private extractSkills(resume: Resume): string[] {
    ...
    const allText =
      `${resume.skills?.join(' ') || ''} ${resume.workExperience || ''} ${resume.projects || ''}`.toLowerCase();
  ...
  private calculateExperienceYears(resume: Resume): number {
    ...
    matches.forEach(match => {
      ...
      if (startYear) {
        ...
        const end =
          endYear && !['现在', '至今'].includes(endYear) ? parseInt(endYear) : currentYear;
  ...
  private extractIndustryExperience(resume: Resume): string[] {
    ...
    const allText =
      `${resume.workExperience || ''} ${resume.projects || ''} ${resume.summary || ''}`.toLowerCase();
  ...
  private extractJobLevel(job: Job): string {
    ...
    if (text.includes('专家') || text.includes('architect') || text.includes('principal'))
      return '专家';
...
class HybridExtractionService {
  ...
  private mergeResults(
    ...
    return {
      ...
      experienceYears: Math.max(traditional.experienceYears ?? 0, ai.experienceYears ?? 0),
      educationLevel: ai.educationLevel || traditional.educationLevel,
      industryExperience: [
        ...new Set([...traditional.industryExperience, ...ai.industryExperience]),
      ],
      location: ai.location || traditional.location,
      keyAchievements: [...new Set([...traditional.keyAchievements, ...ai.keyAchievements])],
      salaryFlexible: ai.salaryFlexible ?? traditional.salaryFlexible,
    };
  }
}
