...
import redis, { getRedisMode } from '../utils/redis';
...
async function storeNonce(
  address: string,
  nonce: string,
  expireSeconds: number = 15 * 60
): Promise<void> {
  const key = `auth:nonce:${address.toLowerCase()}`;
  await redis.set(key, nonce, 'EX', expireSeconds);
  console.log(`存储nonce成功: ${key}`);
}
...
router.get('/challenge', async ctx => {
  ...
  try {
    ...
    // 存储nonce，15分钟过期
    await storeNonce(address, nonce, NONCE_TTL_SECONDS);
...
router.post('/verify-signature', async ctx => {
  // 明确类型，防止类型报错
  const { address, signature, message } = ctx.request.body as {
    address: string;
    signature: string;
    message: string;
  };
