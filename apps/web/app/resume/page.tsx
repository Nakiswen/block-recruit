...
export default function ResumePage() {
  ...
  const createJob = async (formData: FormData) => {
    try {
      ...
      console.log('使用认证头发送请求:', authHeaders); // 调试日志
  ...
  const handleSubmitResume = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!success) {
        throw new Error(responseError || '简历解析失败')
      }
      
      router.push(`/resume/${data.analysis.id}`)
    } catch (error: any) {
      // 处理401未授权错误
      if (error.message?.includes('401') || error.message?.includes('请先连接钱包')) {
        setError('请先连接钱包以上传简历')
          ...
          <form onSubmit={e => {
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm text-gray-600 mb-1">技能权重</label>
                  <input
                    type="number"
                    name="skillWeight"
                    className="w-full p-2 border rounded"
                ...
                <div>
                  <label className="block text-sm text-gray-600 mb-1">经验权重</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-sm text-gray-600 mb-1">Web3权重</label>
                  <input
                    type="number"
                    name="web3Weight"
                    className="w-full p-2 border rounded"
