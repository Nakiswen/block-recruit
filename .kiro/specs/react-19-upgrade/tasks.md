...
- [ ] 2. 更新 apps/web 的核心依赖
  - [ ] 2.1 更新 apps/web/package.json 中的 Next.js 和 React
...
- [ ] 3. 更新 packages/ui 的依赖
  - [ ] 3.1 更新 packages/ui/package.json
...
- [ ] 6. 处理 Next.js 15 破坏性变更
  ...
  - [ ] 6.2 检查并更新 fetch 缓存策略
    - 搜索所有服务器端的 `fetch` 调用
    - 如需缓存，显式添加 `{ cache: 'force-cache' }`
    - 验证缓存行为符合预期
    - _需求: 3.3_
  
  - [ ] 6.3 验证 next.config.js 配置
...
- [ ] 8. 处理 @headlessui/react 2.0 升级
  - [ ] 8.1 检查 Headless UI 破坏性变更
    - 查看 @headlessui/react 2.0 的迁移指南
    - 识别项目中使用的 Headless UI 组件
    - 更新组件使用方式以匹配新 API
    - _需求: 2.2_

- [ ] 9. 处理 framer-motion 11.0 升级
  - [ ] 9.1 检查 Framer Motion 破坏性变更
    - 查看 framer-motion 11.0 的变更日志
    - 识别项目中使用的动画组件
    - 更新动画配置以匹配新 API
    - _需求: 2.3_

- [ ] 10. 修复 TypeScript 类型错误
  - [ ] 10.1 运行类型检查
